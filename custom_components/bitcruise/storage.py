...
from .plan_state import StoredState, stored_state_from_dict, stored_state_to_dict
...
class PlanStore:
    ...
    async def async_load(self) -> StoredState:
        """Read the record, the switch and the approval policy.
    ...
    async def async_save(self, state: StoredState) -> None:
        """Write the state."""
        await self._store.async_save(stored_state_to_dict(state))
